    ...
    public ChangeRequestSpecBuilder makeSequel(ChangeRequestSpecBuilder builder) {
        ...
        .change_request("custom product when initial").zh_CN("自定义产品")
            ...
            .contains_event("product base info").zh_CN("产品基础信息")
                ...
                .has_field("product alias name").zh_CN("产品自命名")
                    .place_holder("请输入产品自命名")
                    .range(0,40).optional()
                .has_field("product legacy id").zh_CN("产品自编码")
                    .place_holder("请输入产品自编码")
                    .range(0,100).optional()
        ...
        .change_request("update product info").zh_CN("更新产品信息")
            ...
            .contains_event("update product info").zh_CN("更新产品信息").as("A")
                .has_field("product id").zh_CN("ID")
                    .fill_by_request("product id")
                    .hidden()
                .has_field("operation type").zh_CN("类型")
                    .fill_by_request("scene code")
                    .hidden()
                .has_field("product standard name").zh_CN("产品名称")
                    .input_at_next_line()
                    .fill_by_request("product id", MODEL.gasProduct().name())
                    .display()
                .has_field("product name").zh_CN("产品自命名")
                    .input_at_next_line()
                    .fill_by_request("product id", MODEL.gasProduct().nickName())
                    .range(1, 100).optional()
                .has_field("legacy code").zh_CN("产品自编码")
                    .fill_by_request("product id", MODEL.gasProduct().legacyCode())
                    .optional()
                    .range(0,100)
                .has_field("comment").zh_CN("备注")
                    .place_holder("变更记录/原因")
                    .optional()
                    .range(0,100)

        .change_request("update inventory parameter").zh_CN("安全库存设置")
            .step("A").zh_CN("更新")
            .contains_event("update inventory parameter").zh_CN("更新安全库存设置").as("A")
                .has_field("product id").zh_CN("ID")
                    .fill_by_request("product id")
                    .hidden()
                .has_field("target quantity").zh_CN("安全库存目标数量")
                    .input_at_next_line()
                    .fill_by_request("product id", MODEL.gasProduct().targetQuantity())
                    .which_type_of(FieldType.INTEGER)
                    .range(0, 1000000)
                    .place_holder("请输入备库目标数量")
                .has_field("threshold quantity").zh_CN("安全库存下限预警")
                    .input_at_next_line()
                    .fill_by_request("product id", MODEL.gasProduct().thresholdQuantity())
                    .which_type_of(FieldType.INTEGER)
                    .range(0, 1000000)
                    .place_holder("请输入库存预警下限")
                .has_field("quantity per day").zh_CN("预期每天可销售量")
                    .input_at_next_line()
                    .fill_by_request("product id", MODEL.gasProduct().quantityPerDay())
                    .which_type_of(FieldType.INTEGER)
                    .range(0, 1000000)
                    .place_holder("请输入预期每天可销售量")
